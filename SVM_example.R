######################
# Tutorial about SVM #
# 2015/07/09         #
######################
# This tutorial follows:
# http://www.r-bloggers.com/learning-kernels-svm/
#
# We only study 2-class SVM for classification here.
#
# For 1-class SVM for anomaly detection, there is a good introduction here:
# http://rvlasveld.github.io/blog/2013/07/12/introduction-to-one-class-support-vector-machines/
# One-Class SVM according to Tax and Duin is easier to understand,
# and can be computed with kernal or e1071 packages, for example using:
# library(e1071)
#     svm.model<-svm(trainpredictors, y=NULL,
#                    type='one-classification',
#                    nu=0.10,
#                    scale=TRUE,
#                    kernel="radial")
#
# Let's talk about classic SVM now:
library(kernlab)

kfunction <- function(linear = 0, quadratic = 0) {
  k <- function (x,y) {
    linear * sum((x) * (y)) + quadratic * sum((x ^ 2) * (y ^ 2))
  }
  class(k) <- "kernel"
  k
}

##########################
# SVM with linear kernel #
##########################

##
# Sample data
##
n = 25
a1 = rnorm(n)
a2 = 1 - a1 + 2 * runif(n)
b1 = rnorm(n)
b2 = -1 - b1 - 2 * runif(n)
x = rbind(matrix(cbind(a1,a2),,2), matrix(cbind(b1,b2),,2))
y <- matrix(c(rep(1,n), rep(-1,n)))

##
# Train model
##
svp <- ksvm(x,y,
            type = "C-svc",
            C = 100,
            # kernel here is scalar product <.|.> induced by Euclidian norm
            kernel = kfunction(1,0),
            scaled = c())

##
# Extract w and b from the model
##
## w is normal vector to the hyperplane that separates vectors
# Defined with \mathbf{w} = \sum_{i=1}^n {\alpha_i y_i\mathbf{x_i}} 
#                         = \sum_{i \in SVindex(svp)} {y_i\mathbf{x_i}}.
# Where alpha_i = 1 when it is a support vector and 0 else. 
# Here only 3 support vectors (cf SVindex(svp)), then only sum of those 3.
# Then, coeff related y in coef(svp)[[1]].
w <- colSums(coef(svp)[[1]] * x[SVindex(svp),])

## b = the offset = ecartement. 
# See here en.wikipedia.org/wiki/File:Svm_max_sep_hyperplane_with_margin.png
b <- b(svp)

##
# Plot results
##
plot(c(min(x[,1]), max(x[,1])),
     c(min(x[,2]), max(x[,2])),
     type = 'n',xlab = 'x1',ylab = 'x2')
title(main = 'Linear Separable Features')
ymat <- ymatrix(svp)

## Plot sample and bold support vectors (3 here)
points(x[-SVindex(svp),1], x[-SVindex(svp),2], 
       pch = ifelse(ymat[-SVindex(svp)] < 0, 2, 1))
points(x[SVindex(svp),1], x[SVindex(svp),2], 
       pch = ifelse(ymat[SVindex(svp)] < 0, 17, 16))

## Draw the lines: hyperplane separation and margin hyperplanes

# equivalent to the hyperplane equation: <w|x> - b = 0 (verified)
abline(b / w[2],-w[1] / w[2])

# See image en.wikipedia.org/wiki/File:Svm_max_sep_hyperplane_with_margin.png
# to understand b+1
abline((b + 1) / w[2],-w[1] / w[2],lty = 2) 

# See image en.wikipedia.org/wiki/File:Svm_max_sep_hyperplane_with_margin.png
# to understand b-1
abline((b - 1) / w[2],-w[1] / w[2],lty = 2)

# Note: Ideally, only 3 support vectors are necessary here, since in 2D and
# linear. With the constraints, defining 3 points induces a unique separate
# hyperplane.

#############################
# SVM with quadratic kernel #
#############################
# Example where linear kernel will not work, but ok with quadratic kernel.
# The code shows separation both in feature and kernel space.

##
# Sample data
##
n = 20
r = runif(n)
a = 2*pi*runif(n)
a1 = r*sin(a)
a2 = r*cos(a)
r = 2+runif(n)
a = 2*pi*runif(n)
b1 = r*sin(a)
b2 = r*cos(a)
x = rbind(matrix(cbind(a1,a2),,2), matrix(cbind(b1,b2),,2))
y <- matrix(c(rep(1,n), rep(-1,n)))

##
# Train model
##
svp <- ksvm(x,y,type="C-svc",C = 100, kernel=kfunction(0,1),scaled=c())

##
# Extract w and b from the model
##

## w
# Here w = \sum_{i \in SVindex(svp)}{y_i\mathbf{x_i}^2}
w2 <- colSums(coef(svp)[[1]] * x[SVindex(svp),]^2)
# To correct the error in the original code (*)
w_test = w2
w2[1] = w_test[2]
w2[2] = w_test[1]

## b
b <- b(svp)

##
# Plot results
##
par(mfrow=c(1,2))
plot(c(min(x[,1]), max(x[,1])),
     c(min(x[,2]), max(x[,2])),
     type='n', xlab='x1', ylab='x2')
title(main='Feature Space')
ymat <- ymatrix(svp)

## Plot sample and bold support vectors
points(x[-SVindex(svp),1], x[-SVindex(svp),2], 
       pch = ifelse(ymat[-SVindex(svp)] < 0, 2, 1))
points(x[SVindex(svp),1], x[SVindex(svp),2], 
       pch = ifelse(ymat[SVindex(svp)] < 0, 17, 16))

## Draw the lines: circle separation and margin circles
# This curve is: w[2]^2 x^2 + w[1]^2 y^2 = b without (*)
# With (*), it corrects the error and leads to 
# b = <w^2|x^2> = w[1]^2 x^2 + w[2]^2 y^2
x1 = seq(min(x[,1]), max(x[,1]), 0.01)
x2 = seq(min(x[,2]), max(x[,2]), 0.01)
points(-sqrt((b-w2[1]*x2^2)/w2[2]), x2, pch = 16 , cex = .1)
points(sqrt((b-w2[1]*x2^2)/w2[2]), x2, pch = 16 , cex = .1)
points(x1, sqrt((b-w2[2]*x1^2)/w2[1]), pch = 16 , cex = .1)
points(x1,  -sqrt((b-w2[2]*x1^2)/w2[1]), pch = 16, cex = .1)

# Some warnings, but all is OK.
points(-sqrt((1+ b-w2[1]*x2^2)/w2[2]) , x2, pch = 16 , cex = .1)
points( sqrt((1 + b-w2[1]*x2^2)/w2[2]) , x2,  pch = 16 , cex = .1)
points( x1 , sqrt(( 1 + b -w2[2]*x1^2)/w2[1]), pch = 16 , cex = .1)
points( x1 , -sqrt(( 1 + b -w2[2]*x1^2)/w2[1]), pch = 16, cex = .1)

points(-sqrt((-1+ b-w2[1]*x2^2)/w2[2]) , x2, pch = 16 , cex = .1)
points( sqrt((-1 + b-w2[1]*x2^2)/w2[2]) , x2,  pch = 16 , cex = .1)
points( x1 , sqrt(( -1 + b -w2[2]*x1^2)/w2[1]), pch = 16 , cex = .1)
points( x1 , -sqrt(( -1 + b -w2[2]*x1^2)/w2[1]), pch = 16, cex = .1)

##
# Showing data on kernel space
##
# From feature space, we are going to the kernel space through the
# kernel function (here x |--> x^2). Then we apply hyperlane separation
# in this kernel space, and we come back to feature space.
xsq <- x^2
svp <- ksvm(xsq,y,type="C-svc",C = 100, kernel=kfunction(1,0),scaled=c())

plot(c(min(xsq[,1]), max(xsq[,1])),
     c(min(xsq[,2]), max(xsq[,2])),
     type='n', xlab='x1^2', ylab='x2^2')
title(main='Quadratic Kernel Space')
ymat <- ymatrix(svp)
points(xsq[-SVindex(svp),1], xsq[-SVindex(svp),2], 
       pch = ifelse(ymat[-SVindex(svp)] < 0, 2, 1))
points(xsq[SVindex(svp),1], xsq[SVindex(svp),2], 
       pch = ifelse(ymat[SVindex(svp)] < 0, 17, 16))

# Extract w and b from the model   
w <- colSums(coef(svp)[[1]] * xsq[SVindex(svp),])
b <- b(svp)

# Draw the lines
abline(b/w[2],-w[1]/w[2])
abline((b+1)/w[2],-w[1]/w[2],lty=2)
abline((b-1)/w[2],-w[1]/w[2],lty=2)

############################
# SVM with a custom kernel #
############################
# In this sample, data can be separated with a parabol.
# Note that quadratic kernel will fail to perform separation, according to
# last plot of the code.
# Idea is to take a custom kernel: not linear, not square, but a combination
# (so it is a polynomial of order 2)

##
# Sample data
##
n = 100
a1 = 4*runif(n)-2
a2 = 1 - a1^2 - 0.5 - 2*runif(n)
b1 = 4*runif(n)-2
b2 = 1 - b1^2 + 0.5 + 2*runif(n)

x = rbind(matrix(cbind(a1,a2),,2),matrix(cbind(b1,b2),,2))
y <- matrix(c(rep(1,n),rep(-1,n)))

##
# Train model
##

## Create the custom kernel
center_kernel <- function(kernel)
{
  m <- length(kernel[,1])
  ones <- matrix(rep(1,m),m)
  (diag(m) - ones %*% t(ones) / m)*kernel*(diag(m) - ones %*% t(ones) / m)
}

f_product<- function(x,y)
{
  sum(diag(crossprod(t(x),y)))
}

f_norm<- function(x)
{
  sqrt(f_product(x,x))
}

kernel_alignment <- function(x,y)
{
  f_product(x,y)/(f_norm(x)*f_norm(y))
}

x_kernel1 <- kernelMatrix(kfunction(1,0),x)
y_kernel <- y %*% t(y)
x_kernel2 <- kernelMatrix(kfunction(0,1),x)

x_kernel1_c <- center_kernel(x_kernel1)
x_kernel2_c <- center_kernel(x_kernel2)

y_kernel_c <- center_kernel(y_kernel)

alignment1 <- kernel_alignment(x_kernel1_c,y_kernel_c)
alignment2 <- kernel_alignment(x_kernel2_c,y_kernel_c)

kernel = kfunction(alignment1,alignment2)

## Train model
svp <- ksvm(x,y,type="C-svc", C = 100, kernel=kernel, scaled=c())

##
# Extract w and b from the model
##
## w
w <- colSums(coef(svp)[[1]] * (alignment1*x[SVindex(svp),]))
v <- colSums(coef(svp)[[1]] * (alignment2*x[SVindex(svp),]^2))

## b
b <- b(svp)

##
# Plot results
##
par(mfrow=c(1,2))
plot(c(min(x[]), max(x[])), c(min(x[]), max(x[])),type='n',xlab='x1',ylab='x2')
title(main='Parabolic Featureset (Mixed Kernel SVM)')
ymat <- ymatrix(svp)

## Plot sample and bold support vectors
points(x[-SVindex(svp),1], x[-SVindex(svp),2], 
       pch = ifelse(ymat[-SVindex(svp)] < 0, 2, 1))
points(x[SVindex(svp),1], x[SVindex(svp),2], 
       pch = ifelse(ymat[SVindex(svp)] < 0, 17, 16))

## Draw the lines: parabol separation and margin parabols
x1 = seq(min(x[]),max(x[]),0.01)
x2 = seq(min(x[]),max(x[]),0.01)

# Some warnings, but all is OK.
points(sqrt( (b-w[2]*x2-v[2]*x2^2)/v[1] + (w[1]/(2*v[1]))^2 ) - w[1]/(2*v[1]) , x2, pch = 16, cex = .1 )
points(-sqrt( (b-w[2]*x2-v[2]*x2^2)/v[1] + (w[1]/(2*v[1]))^2 ) - w[1]/(2*v[1]) , x2, pch = 16, cex = .1 )
points(x1, sqrt( (b-w[1]*x1-v[1]*x1^2)/v[2] + (w[2]/(2*v[2]))^2 ) - w[2]/(2*v[2]) , pch = 16, cex = .1 )
points(x1, -sqrt( (b-w[1]*x1-v[1]*x1^2)/v[2] + (w[2]/(2*v[2]))^2 ) - w[2]/(2*v[2]) , pch = 16, cex = .1 )

points(sqrt( (1+ b-w[2]*x2-v[2]*x2^2)/v[1] + (w[1]/(2*v[1]))^2 ) - w[1]/(2*v[1]) , x2, pch = 16, cex = .1 )
points(-sqrt( (1 + b-w[2]*x2-v[2]*x2^2)/v[1] + (w[1]/(2*v[1]))^2 ) - w[1]/(2*v[1]) , x2, pch = 16, cex = .1 )
points(x1, sqrt( (1+b-w[1]*x1-v[1]*x1^2)/v[2] + (w[2]/(2*v[2]))^2 ) - w[2]/(2*v[2]) , pch = 16, cex = .1 )
points(x1, -sqrt( (1+b-w[1]*x1-v[1]*x1^2)/v[2] + (w[2]/(2*v[2]))^2 ) - w[2]/(2*v[2]) , pch = 16, cex = .1 )

points(sqrt( (-1+ b-w[2]*x2-v[2]*x2^2)/v[1] + (w[1]/(2*v[1]))^2 ) - w[1]/(2*v[1]) , x2, pch = 16, cex = .1 )
points(-sqrt( (-1 + b-w[2]*x2-v[2]*x2^2)/v[1] + (w[1]/(2*v[1]))^2 ) - w[1]/(2*v[1]) , x2, pch = 16, cex = .1 )
points(x1, sqrt( (-1+b-w[1]*x1-v[1]*x1^2)/v[2] + (w[2]/(2*v[2]))^2 ) - w[2]/(2*v[2]) , pch = 16, cex = .1 )
points(x1, -sqrt( (-1+b-w[1]*x1-v[1]*x1^2)/v[2] + (w[2]/(2*v[2]))^2 ) - w[2]/(2*v[2]) , pch = 16, cex = .1 )

## Checking that data is not separable using square kernel
plot(x[]^2,type='n',xlab='x1^2',ylab='x2^2')
title(main='Not Linear Separable in Quadratic Kernel Coordinates')
points(x[1:100,]^2,pch=16)
points(x[101:200,]^2,pch=2)
